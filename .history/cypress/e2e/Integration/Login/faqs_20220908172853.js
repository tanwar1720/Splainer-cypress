describe('faqs_sweets',()=>
{
    it ('faq-page',()=>
    {
      ...
      //  cy.get('.card-body').eq(1).should('be.visible')
        ...
        // cy.go('back')
        routting('Sign up here!')
        routting('Terms & Conditions')
        // cy.get('.container').eq(2).find('a').contains('Terms & Conditions').click()
        // cy.go('back')
        const arr = []
        cy.get('.container').eq(2).find('a').each(($el)=>
        {
            routting(cy.wrap($el).text())
            // cy.log(arr)
        })

          
        // cy.get('.container').eq(2).find('a').contains('Sign ')
...
function routting(text){
    cy.get('.container').eq(2).find('a').contains(text).click()
    cy.go('back')
}