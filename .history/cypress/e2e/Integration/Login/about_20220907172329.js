    ...
    it('Show_aboutPage',() => {
        ...
        {  
            //  const name = $el.contains('Lakshmi Chaudhry','Sara Varghese','Rachel John','Ayaan Malhotr','Anushree Kaushal')
             ...
             if($el.text() === 'Lakshmi Chaudhry','Sara Varghese','Rachel John','Ayaan Malhotr','Anushree Kaushal'){
                cy.log($el.text())
             }
        })
        cy.get('.sc').should('be.visible').find('img')})

        it('Show_Anchor',()=>
        {
            cy.get('a').each(($el)=>
            {
               if($el.text().to.have.attr("href").not.contain("undefined"))
               {
                cy.log('Yes contain Anchor link')
               }
               else{
                cy.log('Not found')
               }
